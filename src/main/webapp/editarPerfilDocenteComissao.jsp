...
<head>
    ...
    <script>
        function onEditField(field){
            if ('name' === field){
                $('input[id^=nomeDocenteComissao]').prop('disabled',false);
                $('button[id^=editFieldName]').prop('hidden',true);
            }else if ('siape' === field){
                $('input[id^=siapeDocenteComissao]').prop('disabled',false);
                $('button[id^=editFieldSiape]').prop('hidden',true);
            }
        }
        function onEditDocente(){
            let formEditDocenteComissao = document.getElementById('docenteComissaoForm');
            if(formEditDocenteComissao) {
                if (!formEditDocenteComissao.checkValidity()) {
                    formEditDocenteComissao.classList.add('was-validated');
            ...
            $.ajax({
                ...
                cache: false,
                data: {
                    submitButtonEditDocenteComissao: $('button[id^=submitButtonEditDocenteComissao]').val(),
                    nomeDocenteComissao: $('input[id^=nomeDocenteComissao]').val(),
                    siapeDocenteComissao: $('input[id^=siapeDocenteComissao]').val()
                },
                dataType: "json",
                success: function (data) {
                    if (data.editarDocenteComissao) {
                        alert('Docente editado com sucesso!');
                        window.location.replace('editarPerfilDocenteComissao.jsp');
                    }
                },
        ...
        function onDeleteDocente(){
            $.ajax({
                ...
                cache: false,
                data: {
                    submitButtonExcluirDocenteComissao: $('button[id^=submitButtonExcluirDocenteComissao]').val()
                },
                dataType: "json",
                success: function (data) {
                    if (data.excluirDocenteComissao) {
                ...
                <div class="sw sw-theme-basic sw-justified">
                    <div class="tab-content">
                        <div class="tab-pane" style="display: block">
                            <form class="needs-validation" novalidate autocomplete="off" id="docenteComissaoForm">
                                <label for="nomeDocenteComissao">Nome</label>
                                <div class="input-group mb-3">
                                    <input class="form-control" id="nomeDocenteComissao" type="text" name="nomeDocenteComissao" required disabled value="${DOCENTE_COMISSAO.nome}"/>
                                ...
                                <div>
                                    <label for="siapeDocenteComissao">Matrícula SIAPE</label>
                                </div>
                                <div class="input-group mb-3">
                                    <input class="form-control" id="siapeDocenteComissao" type="text" name="siapeDocenteComissao" required disabled value="${DOCENTE_COMISSAO.siape}"/>
                                    <div class="input-group-append">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-primary" type="button" id="submitButtonExcluirDocenteComissao" name="submitButtonExcluirDocenteComissao" value="excluirDocenteComissao" onclick="onDeleteDocente()">Sim</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="confirmModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="confirmModalLabel">Editar Docente da Comissao?</h5>
                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary" type="button" data-dismiss="modal">Não</button>
                <button class="btn btn-primary" type="button" id="submitButtonEditDocenteComissao" name="submitButtonEditDocenteComissao" value="editarDocenteComissao" onclick="onEditDocente()">Enviar</button>
            </div>
        </div>
    </div>
</div>
